...
export function PlayerPaymentsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const pmtRecs = await getPlayerPayments(profile.uid);
        const hasPaid = pmtRecs.some(p => p.season === '2023-2024');
  ...
  const handlePayment = async () => {
    ...
    setTimeout(async () => {
      try {
        await recordPayment(profile.uid!, profile.clubId!, 120, '2023-2024');
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm">
        ...
        {paymentSuccess ? (
          <div className="flex flex-col items-center justify-center p-8 bg-emerald-50 rounded-2xl border border-emerald-100 text-center">
            ...
            <p className="text-sm text-emerald-700 mt-1">El club ha recibido el pago correspondiente a la temporada actual.</p>
          </div>
        ) : (
          <div className="flex flex-col sm:flex-row items-center justify-between p-5 rounded-2xl border border-slate-200 bg-slate-50 gap-4">
            <div className="flex items-center gap-4 w-full sm:w-auto">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 mt-0.5">Temporada actual</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all">
            <div className="p-6 bg-slate-50 border-b border-slate-100 text-center">
              ...
              <p className="text-sm text-slate-500 mt-1">Abono de cuota de club</p>
            </div>
            <div className="p-8">
              <div className="flex justify-between items-center mb-8 pb-6 border-b border-slate-100">
                <span className="font-semibold text-slate-600">Total a pagar:</span>
                <span className="text-3xl font-black text-slate-900">120,00 €</span>
              </div>
              ...
              >
                {isPaying ? (<><Loader2 className="w-6 h-6 animate-spin" /> Procesando...</>) : 'Pagar 120€ de forma segura'}
              </button>
