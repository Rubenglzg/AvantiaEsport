...
export function ClubAnnouncementsPage() {
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    try { setAnnouncements(await getClubAnnouncements(profile.uid)); }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setFormLoading(true);
    try {
      await createAnnouncement({ title, body, pinned, authorId: profile.uid, authorName: profile.name || 'Club', scope: 'club', clubId: profile.uid, createdAt: new Date().toISOString() });
      setShowModal(false); setTitle(''); setBody(''); setPinned(false);
      await loadData();
    } catch (e) { console.error(e); alert("Error al crear comunicado."); }
    finally { setFormLoading(false); }
  };

  const handleDelete = async (id: string) => {
    if (!window.confirm('¿Eliminar este comunicado?')) return;
    try { await deleteAnnouncement(id); await loadData(); }
    catch (e) { console.error(e); }
  };
              ...
              <div key={a.id} className={`p-5 rounded-2xl border transition-colors hover:shadow-sm ${a.pinned ? 'border-amber-300 bg-amber-50/50' : 'border-slate-200 bg-white'}`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-400 mt-3 font-semibold">{formatDate(a.createdAt)}</p>
                  </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-5">
              <div>
                ...
                <input type="text" required value={title} onChange={e => setTitle(e.target.value)} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-amber-500 transition-all" placeholder="Ej. Cambio de horario" />
              </div>
              <div>
